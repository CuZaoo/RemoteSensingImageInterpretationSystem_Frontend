<template>
  <div>
    <el-table :data="bhjc_img_list_results" style="width: 100%" max-height="600"
      ...
      <el-table-column label="图片2" :width="280">
        <template slot-scope="scope">
          <span v-if="scope.row.img2">
            <el-tooltip class="item" effect="dark" content="点击图片查看原图" placement="top">
              <el-image
          ...
          <span v-else>
            <el-upload class="avatar-uploader" :action="imagesUploadApi" :show-file-list="false" :on-exceed="exceed"
                       :on-success="(value)=> uploadSuccess_all2(scope.$index, value)">
              <i class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </span>
        </template>
      </el-table-column>
      <el-table-column label="处理图" :width="255">
        <template slot-scope="scope">
      ...
      <el-table-column label="模型" align="right">
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="选择处理单个图片的模型" placement="top">
            <el-select v-model="selectedModelID[scope.$index]" placeholder="待运行模型" popper-class="selectJob" @change="selectChange"
        ...
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="运行" placement="top">
            <el-button type="success" icon="el-icon-s-promotion" plain
                       @click="detect(scope.$index,scope.row)"></el-button>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  watch: {
    bhjc_img_list_result(newVal, oldVal) {
      console.log('上传完图片')
      this.bhjc_img_list_results = this.bhjc_img_list_results.map((item, i) => {
        this.$set(item, 'result', newVal[i])
        console.log(item)
        return item
      })
    },
    bhjc_img_list(newVal, oldVal) {
      console.log('上传完图片2')
      this.bhjc_img_list_results = newVal.map(item => {
        return { img1: item.img1,img2: item.img2 }
      })
    }
  },
  methods: {
    ...
    detect(index, row) {
      this.$confirm('是否处理当前图片?', '提示', {
        ...
        if (row === undefined || row.img1 === undefined || row.img2 === undefined) {
          this.showHelp = true
        ...
        //前端回显需要修改的地方，bhjc_img_list，不再是一个普通数组，而是一个包含对象的数组
        //2.多图全部运行：
        //前端需要传参，一个包含对象的数组，对象包含图的index，图的url
        // 后端接受参数，通过for循环将url逐个处理
        //后端返回一个数组包含对象，前端应该直接将接受的数组取代bhjc_img_list
        this.showHelp = false
        ...
        run(row.img1, row.img2, types, modelIds, userId).then(res => {
          console.log('单个检测')
          console.log(index)
          console.log(res)
          this.$store.dispatch('set_img_to_bhjc_result_index', {
            'index': index,
    ...
    uploadSuccess_all2(index,response, file, fileList) {
  ...
  mounted() {
    this.bhjc_img_list_results = this.bhjc_img_list.map(item => {
      return { img1: item.img1,img2: item.img2 }
    })

    this.bhjc_img_list_results = this.bhjc_img_list_results.map((item, i) => {
      this.$set(item, 'result', this.bhjc_img_list_result[i])
      return item
    })
  }
}
