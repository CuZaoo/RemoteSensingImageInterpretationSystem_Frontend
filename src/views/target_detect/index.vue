        ...
        <el-col :xs="24" :sm="24" :lg="24">
          <el-card :style="{height:windowHeight - 160+'px'}">
              ...
              <el-row :gutter="40">
                <el-col :xs="24" :sm="12" :lg="12">
                  <el-card class="card_style" :body-style="{ height:windowHeight - 280+'px' }" style="overflow: auto">
                    <div v-if="mbjc_img_list === null || mbjc_img_list[0] === undefined" class="centerVertically">
                      <div style="padding: 14px;">
                        <span style="font-size: 25px">待 检 测 图</span>
                      </div>
                      <el-popover
                        trigger="manual"
                        placement="left"
                    ...
                    <div v-else style="overflow: auto">
                      <el-tooltip class="item" effect="dark" content="点击图片查看大图" placement="top">
                        <el-image
                          :preview-src-list="[mbjc_img_list[0]]"
                ...
                <el-col :xs="24" :sm="12" :lg="12">
                  <el-card class="card_style" :body-style="{ height:windowHeight - 280+'px'}" style="overflow: auto">
                    <div
                      v-if="!is_detect  &&  (mbjc_img_list_results === null || mbjc_img_list_results[0] === undefined)"
                    ...
                    <div v-else-if=" mbjc_img_list_results === null || mbjc_img_list_results[0] === undefined">
                    ...
                    <div v-else style="overflow: auto">
                      <el-image
                        :preview-src-list="[mbjc_img_list_results[0]]"
                        :style="{height:windowHeight - 310+'px'}"
                        :src="mbjc_img_list_results[0]"
                        fit="fill"
                      ></el-image>
                    </div>
                  </el-card>
            ...
            <div style="padding: 10px">
              <div>
                <el-popover
                  v-if="mbjc_img_list_results === null || mbjc_img_list_results[0] === undefined"
                  trigger="manual"
                  placement="bottom"
                  title="运行"
                  width="200"
                ...
                >
                        <span slot="reference">
                          <el-button type="success" @click="detect(mbjc_img_list[0])"><i
                ...
                >
                        <span slot="reference">
                          <el-button type="success" @click="downloadByUrl(mbjc_img_list_results[0],'download')"><i
...
<script>
import Table from '@/views/target_detect/components/Table'
...
export default {
  ...
  watch: {
    mbjc_img_list_result(newVal, oldVal) {
      this.mbjc_img_list_results = newVal
    }
  },
  mounted() {
    var that = this
    ...
    this.mbjc_img_list_results = this.mbjc_img_list_result
    //  获取最新的运行统计
    // getStatCount(4).then(res => {
    //   this.mbjc_nums = res.count
    // })
...
//刷新提示
  ...
  methods: {
    ...
    detect(img) {
      ...
      this.$confirm('是否开始检测?', '提示', {
        ...
        run(img, 0, 0, userId).then(res => {
          this.$store.dispatch('set_img_to_mbjc_result', res.img_result_url)
    ...
    uploadSuccess_solo(response, file, fileList) {
      this.showHelp = false
      this.is_img1 = true
      this.$store.dispatch('set_img_to_mbjc', response.path)
    },
    mouseOver(flag) {
      if (flag) {
    ...
    del_img() {
      this.$store.dispatch('del_img_to_mbjc_all')
      this.is_img1 = false
      this.is_img2 = false
      this.mbjc_img_list_results = null
    },
    mouseLeave() {
    ...
    handleRunModel() {
      ...
      if (this.mbjc_img_list.length === 0) {
      ...
      this.$confirm('是否处理全部图片?', '提示', {
        ...
        // let userId = 0
        runAll(this.mbjc_img_list, 1, modelIds, userId).then(res => {
          this.mbjc_all_run = true
          this.$store.dispatch('set_img_to_mbjc_result_all', res.img_result_urls)
          this.$message.success('提取完毕')
        }).catch(err => {
          this.$message.error('提取异常')
        })
    ...
    loadFileList() {
      let arr = this.mbjc_img_list_result.map((item, i) => {
    ...
    getTableList(val) {
      if (val) {
        getModelByPage(3, this.page, this.size).then((res) => {
