        ...
        <el-col :xs="24" :sm="24" :lg="leftCol">
          <el-card :style="{height:windowHeight - 160+'px'}">
              ...
              <el-row :gutter="40">
                <el-col :xs="24" :sm="12" :lg="12">
                  <el-card class="card_style" :body-style="{ height:windowHeight - 280+'px' }" style="overflow: auto">
                    <div v-if="mbtq_img_list[0] === undefined" class="centerVertically">
                      <div style="padding: 14px;">
                        <span style="font-size: 25px">待 提 取 图</span>
                      </div>
                      <el-popover
                        trigger="manual"
                        placement="left"
                      ...
                      >
                        <span slot="reference">
                          ...
                          <el-progress v-if="progressStatus" :text-inside="true" :stroke-width="26"
                    ...
                    <div v-else style="overflow: auto">
                      <el-tooltip class="item" effect="dark" content="点击图片查看大图" placement="top">
                        <el-image
                          :preview-src-list="[mbtq_img_list[0]]"
                ...
                <el-col :xs="24" :sm="12" :lg="12">
                  <el-card class="card_style" :body-style="{ height:windowHeight - 280+'px'}" style="overflow: auto">
                    <div v-if="!is_detect && (mbtq_img_list_results === null || mbtq_img_list_results[0] === undefined)"
                    ...
                    <div v-else-if="mbtq_img_list_results === null || mbtq_img_list_results[0] === undefined">
                      <img :src="loadingImg" alt=""
                           :style="{height:windowHeight - 340+'px',width:windowHeight - 140+'px'}"
                      >
                      <span style="position: absolute;left: 33%;top: 45%;font-size: 70px" class="_loading"
                      >L o a d i n g</span>
                    </div>
                    <div v-else style="overflow: auto">
                      <el-image
                        :preview-src-list="[mbtq_img_list_results[0]]"
                        :style="{height:windowHeight - 310+'px'}"
                        :src="mbtq_img_list_results[0]"
                        fit="fill"
                      ></el-image>
                    </div>
                  </el-card>
            ...
            <div style="padding: 10px">
              <div>
                <el-popover
                  v-if="mbtq_img_list_results === null || mbtq_img_list_results[0] === undefined"
                  trigger="manual"
                  placement="bottom"
                  title="运行"
                  width="200"
                ...
                >
                        <span slot="reference">
                          <el-button type="success" @click="detect(mbtq_img_list[0])"><i class="el-icon-s-promotion"
                          ></i> 提取</el-button>
                        </span>
                </el-popover>
                <el-popover
                ...
                >
                        <span slot="reference">
                          <el-button type="success" @click="downloadByUrl(mbtq_img_list_results[0],'download')"><i
...
<script>
import Table from '@/views/target_extra/components/Table'
...
export default {
  ...
  watch: {
    mbtq_img_list_result(newVal, oldVal) {
      this.mbtq_img_list_results = newVal
    }
  },
  mounted() {
    var that = this
    ...
    this.mbtq_img_list_results = this.mbtq_img_list_result


    //刷新提示
    window.onbeforeunload = e => {
      console.log('刷新:', this.modified)
      if (!this.modified) {
        return
      }
      // 通知浏览器不要执行与事件关联的默认动作
  ...
  methods: {
    ...
    detect(img) {
      ...
      this.$confirm('是否开始提取?', '提示', {
        ...
        run(img, 0, 0, userId).then(res => {
          this.$store.dispatch('set_img_to_mbtq_result', res.img_result_url)
    ...
    uploadSuccess_solo(response, file, fileList) {
      ...
      this.$store.dispatch('set_img_to_mbtq', response.path)
    },
    mouseOver(flag) {
      if (flag) {
    ...
    del_img() {
      this.modified = false
      this.$store.dispatch('del_img_to_mbtq_all')
      this.is_img1 = false
      this.is_img2 = false
      this.mbtq_img_list_results = null
    },
    mouseLeave() {
    ...
    handleRunAll() {
      ...
      if (this.mbtq_img_list.length === 0) {
      ...
      this.$confirm('是否处理全部图片?', '提示', {
        ...
        // let userId = 0
        runAll(this.mbtq_img_list, 1, modelIds, userId).then(res => {
          this.mbtq_all_run = true
          this.$store.dispatch('set_img_to_mbtq_result_all', res.img_result_urls)
          this.$message.success('提取完毕')
        }).catch(err => {
          this.$message.error('提取异常')
        })
    ...
    loadFileList() {
      let arr = this.mbtq_img_list_result.map((item, i) => {
    ...
    handleSheetChange() {
      // if (this.user.id === undefined) {
      //   this.$message.warning('请“登录”以使用多图处理功能')
      //   return
      // }
    ...
    getTableList(val) {
      if (val) {
        getModelByPage(1, this.page, this.size).then((res) => {
