<template>
  <div>
    <el-table :data="mbtq_img_list_results" style="width: 100%" max-height="600"
        ...
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="运行" placement="top">
            <el-button type="success" icon="el-icon-s-promotion" plain
                       @click="detect(scope.$index,scope.row.img)"></el-button>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { getModelByPage } from '@/api/task/mbtq'
import {
  mapGetters
} from 'vuex'
import { run } from '@/api/task/mbtq'
...
export default {
  ...
  watch: {
    mbtq_img_list_result(newVal, oldVal) {
      console.log('上传完图片')
      this.mbtq_img_list_results = this.mbtq_img_list_results.map((item, i) => {
        this.$set(item, 'result', newVal[i])
        console.log(item)
        return item
      })
    },
    mbtq_img_list(newVal, oldVal) {
      console.log('上传完图片2')
      this.mbtq_img_list_results = newVal.map(item => {
        return { img: item }
      })
    }
  },
  methods: {
    ...
    detect(index, img) {
      this.$confirm('是否处理当前图片?', '提示', {
        ...
        //前端回显需要修改的地方，mbtq_img_list，不再是一个普通数组，而是一个包含对象的数组
        //2.多图全部运行：
        //前端需要传参，一个包含对象的数组，对象包含图的index，图的url
        // 后端接受参数，通过for循环将url逐个处理
        //后端返回一个数组包含对象，前端应该直接将接受的数组取代mbtq_img_list
        this.showHelp = false
        ...
        run(img, types, modelIds, userId).then(res => {
          ...
          console.log(index)
          console.log(res)
          this.$store.dispatch('set_img_to_mbtq_result_index', {
            'index': index,
  ...
  mounted() {
    this.mbtq_img_list_results = this.mbtq_img_list.map(item => {
      return { img: item }
    })

    this.mbtq_img_list_results = this.mbtq_img_list_results.map((item, i) => {
      this.$set(item, 'result', this.mbtq_img_list_result[i])
      return item
    })
  }
}
